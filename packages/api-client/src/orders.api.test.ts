...
describe("orders.api", () => {
  it("should return the orders for a user", async () => {
    const orders = await getOrders({ userId: "user-1" });

    expect(orders.length).toBeGreaterThan(0);
  });

  it("should return a single order by id", async () => {
    const order = await getOrderById({ userId: "user-1", orderId: "order-1" });
  ...
  it("should throw ApiClientError when the cart is empty", async () => {
    server.use(
      http.post("*/api/orders/:userId", () => {
        return HttpResponse.json({ error: { message: "Cart is empty" } }, { status: 400 });
      }),
    );

    await expect(createOrder({ userId: "user-1" })).rejects.toThrow(ApiClientError);
  });
});
