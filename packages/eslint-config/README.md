...
- `test.js`: `eslint-plugin-testing-library` + `@vitest/eslint-plugin`, aplicado a `**/*.test.{ts,tsx}` — detecta selectores no accesibles, tests sin aserciones, `.skip`/`.only` olvidados (`§Testing` del canon).
