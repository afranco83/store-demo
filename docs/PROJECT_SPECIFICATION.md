...
| v1.8    | 2026-07-08 | Adenda a la Fase 3 (revisión post-cierre): el acento gana variante dark (`--color-accent`/`-hover`/`-foreground`/`-soft` bajo `prefers-color-scheme: dark`, ver `ARCHITECTURE.md` §2.1) tras detectar que `#c2410c` no llega a AA sobre fondos oscuros. Resto de la paleta sigue solo-claro; no hay toggle manual de tema todavía                                                                                                          |
| v1.9    | 2026-07-09 | Fase 5 cerrada (Autenticación & Cuenta): la sesión de Auth.js **no** se expone vía Context API/`SessionProvider` como preveía la v1.1 de este documento (§3) — se resuelve con `auth()` server-side y props a los pocos Client Components que lo necesitan, ver `ARCHITECTURE.md` §4; guards documentados como `withAuthGuard()` para `middleware.ts`, sin hook `useRequireAuth()` de cliente (no hizo falta para el alcance de esta fase) |
