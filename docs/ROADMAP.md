...
- **`AUTH_SECRET`/`AUTH_JWT_SECRET` nunca llegaban al proceso de `next start` de los specs E2E en CI, detectado solo al correr de verdad en el runner de GitHub** (mismo tipo de sorpresa que ya avisa `CLAUDE.md`, "Cómo trabajar en este repo", sobre la Fase 4) — causa real de dos capas, no una:
  1. `ci.yml` no definía esas variables en ningún sitio. Primer fix (insuficiente por sí solo): añadirlas como variables de entorno fijas del job (valores de solo-CI, efímeros, no protegen nada real).
  2. La causa de fondo, que el primer fix no tocaba: `apps/storefront`/`apps/admin`'s `playwright.config.ts` pasaban `env: { API_URL: "..." }` al `webServer` de Next.js — la documentación de Playwright dice literalmente _"process.env by default"_, es decir, pasar `env` explícito **reemplaza** todo el entorno heredado en vez de añadirse a él. En local nunca se manifestó porque Next.js carga su propio `.env` igualmente dentro del proceso ya arrancado; en CI no hay ningún `.env` en disco, así que el proceso se quedaba sin `AUTH_SECRET` pese a que el job sí lo tenía. Corregido a `env: { ...process.env, API_URL: "..." }` en ambos archivos.

  Ambos fixes son necesarios y complementarios (el primero pone el valor disponible en el job, el segundo asegura que el proceso hijo lo reciba). Reproducido el fallo exacto en local dos veces (apartando los `.env`, exportando las variables solo por shell) antes de dar cada fix por bueno, sin depender del runner de GitHub para confirmarlo.
