...
**Pendiente, pasos manuales del usuario (no ejecutables por el agente — requieren crear cuentas externas)**: crear 3 proyectos en Vercel desde `afranco83/store-demo` (Root Directory `apps/api`, `apps/storefront`, `apps/admin`) con las variables de entorno correspondientes; verificar el flujo end-to-end contra el despliegue real (checkout en `storefront` visible en la tabla de pedidos de `admin`, prueba de que la persistencia contra Turso funciona entre las dos apps). Hasta que esto se complete, la tarea sigue sin marcar como cumplida en la lista de arriba.
