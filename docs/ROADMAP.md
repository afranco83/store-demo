...
- **`AUTH_SECRET`/`AUTH_JWT_SECRET` ausentes en `ci.yml`, detectado solo al correr de verdad en el runner de GitHub** (mismo tipo de sorpresa que ya avisa `CLAUDE.md`, "Cómo trabajar en este repo", sobre la Fase 4): en local siempre había un `.env` con esos valores generados a mano, así que ningún gate local lo detectó — y al ser la primera vez que `test:e2e` corría en CI, tampoco se había disparado antes. El login real que ejercitan los specs E2E falló con `MissingSecret` de Auth.js en cuanto la PR corrió en GitHub Actions. Corregido añadiendo ambos como variables de entorno fijas del job (valores de solo-CI, efímeros, no protegen nada real) — reproducido el fallo exacto en local apartando los `.env` antes de aplicar el fix, para confirmarlo sin depender del propio runner de GitHub.
