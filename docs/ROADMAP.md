...
- [ ] Decisión y, si procede, despliegue de demo pública (Vercel) — **decisión tomada, migración de código y aprovisionamiento de Turso completados (2026-07-12), despliegue real pendiente de pasos manuales del usuario** (crear los 3 proyectos en Vercel — ver adenda más abajo)
...
**Pendiente, pasos manuales del usuario (no ejecutables por el agente — requieren crear cuentas externas)**: verificar el flujo end-to-end contra el despliegue real ya corregido (checkout en `storefront` visible en la tabla de pedidos de `admin`, prueba de que la persistencia contra Turso funciona entre las dos apps). Hasta que esto se complete, la tarea sigue sin marcar como cumplida en la lista de arriba.
