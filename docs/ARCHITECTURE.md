...
- `release.yml`: se dispara tras el éxito de `ci.yml` sobre `main` (evento `workflow_run`) y corre `semantic-release` (`.releaserc.json` en la raíz) para calcular una versión semver única de todo el monorepo a partir de los Conventional Commits desde el último release (`feat`→minor, `fix`/`perf`→patch, `BREAKING CHANGE`→major), publicar un tag y una GitHub Release, y commitear `CHANGELOG.md` + el bump de `version` en el `package.json` raíz. La versión se expone a `storefront`/`admin` en build time (`NEXT_PUBLIC_APP_VERSION`, inyectada en cada `next.config.ts` desde el `package.json` raíz) y se muestra en el footer de ambas apps enlazando a su Release en GitHub.
