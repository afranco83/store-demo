...
- **Defensa en profundidad**: además del guard de middleware, cada Route Handler de `cart`/`orders` revalida el token independientemente (`apps/api/src/lib/guard.ts`) — el guard de frontend es una capa adicional, no la única barrera de autorización, tal como exige `AGENTS.md §10`.
- `storefront` permite navegación anónima completa del catálogo y del carrito (carrito de invitado vía `X-Guest-Id`, ver §3); solo exige sesión en checkout y "mis pedidos". Al iniciar sesión (login o justo después de un registro), el callback `signIn` de `packages/auth` fusiona el carrito de invitado (cookie `guest_cart_id`) dentro del carrito del usuario (`POST /api/cart/merge`, suma cantidades de productos repetidos) y borra la cookie de invitado.
