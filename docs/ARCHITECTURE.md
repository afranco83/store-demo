...
- Migraciones versionadas (`prisma migrate`) + script de seed con datos de ejemplo (usar `@faker-js/faker` para variedad, con seed fijo para reproducibilidad). Las imágenes de producto del seed se alojan en **Cloudinary** (tier gratuito, subidas vía _unsigned upload preset_ para no requerir el API secret); el campo `imageUrl` de `Product` guarda la URL de Cloudinary, no un binario ni un asset local.
