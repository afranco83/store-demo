...
- **Superficies siempre oscuras (no ligadas a `prefers-color-scheme`) necesitan su propio token, no el de dark mode**: un componente que es oscuro por diseño con independencia del tema del SO (p. ej. el `Footer` de storefront) no puede usar `--color-accent` a secas — ese token solo se aclara cuando el propio SO está en dark, así que con el SO en claro seguiría fallando AA sobre ese fondo oscuro. Se define un token aparte (`--color-accent-on-dark`, mismo valor que la variante dark pero fuera del `@media`) en vez de forzar la superficie a depender de `prefers-color-scheme` o hardcodear el hex en el componente.
